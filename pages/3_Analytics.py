...
from fund import tracker, utils
...
st.caption(
    "Final Prediction may be extreme with little data at first; the value will be omitted for the beginning of the fund. In case of further extreme behavior, a value of N/A will be put in place of values that are either negative or severely infeasible."
)
...
with col1:
    if cfund < 1:
        st.metric(
            label="Current Fund", value=f"{cfund * 1000}K", delta=f"{cfund_delta}K"
        )
    else:
...
with col2:
    if inc_24 and inc_24_pct:
        st.metric(
            label="Change in Past 24 Hours", value=f"{inc_24}K", delta=f"{inc_24_pct}%"
        )
    else:
...
with col3:
    ...
    if (
        final_pred < 0
        or datetime.datetime.now() < cutoff_date
        or final_pred > max(tracker.CHECKPOINTS.keys()) * 1.5
    ):
...
st.caption(
    "Estimated end checkpoint denotes the estimated major checkpoint; we don't care about Mono Apple in this household."
)
...
try:
    ...
    next_week_pred = (
        int(np.round(tracker.predict(cur_day_num + 7, newton=False), -3)) / 10**6
    )
    if cfund >= max(tracker.CHECKPOINTS.keys()):
        ...
        cdelta_raw = mdates.num2date(tracker.newton() + tracker.X_SHIFT).replace(
